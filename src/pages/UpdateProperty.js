...
const UpdateProperty = () => {
    ...
    const idProperty = pathname.split('/')[pathname.split('/').length - 1];

    useEffect(() => {
        axios
            .get(`http://localhost:4000/property/${idProperty}`)
            .then((res) => {
                setOldProperty(res.data);
    ...
    const handleSubmitForm = (e) => {
        ...
        const checkCheckMathOld = (
            ...
            return (
                newname !== oldProperty?.name ||
                newtype !== oldProperty?.type ||
                newdesc !== oldProperty?.description ||
                newprice !== oldProperty?.price ||
                newcountry !== oldProperty?.country ||
                newaddress !== oldProperty?.address ||
                newbedrooms !== oldProperty?.bedrooms ||
                newbathrooms !== oldProperty?.bathrooms ||
                newsurface !== oldProperty?.surface
            );
        };
        ...
        if (
            ...
            if (checkCheckMathOld(name, type, desc, price, country, address, bedrooms, bathrooms, surface)) {
                formRef.current.submit();
            } else {
                alert('Please change at least 1 field !');
            }
        } else {
                ...
                <div className="w-full bg-white rounded-lg shadow dark:border md:mt-0 sm:max-w-md xl:p-0 dark:bg-gray-800 dark:border-gray-700">
                    <div className="p-6 space-y-4 md:space-y-6 sm:p-8">
                        ...
                        <form
                            ...
                            action={`http://localhost:4000/update/${idProperty}`}
                            method="post"
                        ...
                        >
                            ...
                            <div>
                                <label
                                    ...
                                    className="block mb-2 text-sm font-medium text-gray-900 dark:text-white"
                                >
                                    Your property's image :{' '}
                                    {imageFilePath.split('/')[imageFilePath.split('/').length - 1]}
                                </label>
                                <input
                                    type="file"
                                    name="image"
...
export default memo(UpdateProperty);
